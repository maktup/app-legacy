 ...
 **/
 public class UtilBase64{
	...
	/**
	 * Encode using Base64-like encoding that is URL- and Filename-safe as described in Section 4 of RFC3548: <a href="http://www.faqs.org/rfcs/rfc3548.html">http://www.faqs.org/rfcs/rfc3548.html</a>. It is important to note that data encoded this way is <em>not</em> officially valid Base64, or at
	...
	/**
	 * I don't get the point of this technique, but someone requested it, and it is described here: <a href="http://www.faqs.org/qa/rfcc-1940.html">http://www.faqs.org/qa/rfcc-1940.html</a>.
	 */
	...
	public static void encode(java.nio.ByteBuffer raw, java.nio.ByteBuffer encoded) {
		...
		while (raw.hasRemaining()) {
			...
			raw.get(raw3, 0, rem);
			UtilBase64.encode3to4(enc4, raw3, rem, UtilBase64.NO_OPTIONS);
			encoded.put(enc4);
		} // end input remaining
	}
	...
	public static String encodeObject(java.io.Serializable serializableObject, int options) throws java.io.IOException {
		...
		try {
			...
			b64os = new UtilBase64.OutputStream(baos, ENCODE | options);
			if ((options & GZIP) != 0) {
				// Gzip
	...
	public static byte[] encodeBytesToBytes(byte[] source) {
		byte[] encoded = null;
		try {
			encoded = encodeBytesToBytes(source, 0, source.length, UtilBase64.NO_OPTIONS);
		}
		catch (java.io.IOException ex) {
	...
	public static byte[] encodeBytesToBytes(byte[] source, int off, int len, int options) throws java.io.IOException {
		...
		if ((options & GZIP) != 0) {
			java.io.ByteArrayOutputStream baos = null;
			java.util.zip.GZIPOutputStream gzos = null;
			UtilBase64.OutputStream b64os = null;
	...
	/**
	 * Low-level access to decoding ASCII characters in the form of a byte array. <strong>Ignores GUNZIP option, if it's set.</strong> This is not generally a recommended method, although it is used internally as part of the decoding process. Special case: if len = 0, an empty array is returned.
	 * Still, if you need more speed and reduced memory footprint (and aren't gzipping), consider this method.
	 *
	 * @param source
	...
	public static byte[] decode(byte[] source) throws java.io.IOException {
		byte[] decoded = null;
		// try {
		decoded = decode(source, 0, source.length, UtilBase64.NO_OPTIONS);
		// } catch( java.io.IOException ex ) {
		// assert false : "IOExceptions only come from GZipping, which is turned off: " + ex.getMessage();
		// }
		return decoded;
	...
	public static void encodeToFile(byte[] dataToEncode, String filename) throws java.io.IOException {
		...
		UtilBase64.OutputStream bos = null;
		try {
			bos = new UtilBase64.OutputStream(new java.io.FileOutputStream(filename), UtilBase64.ENCODE);
	...
	public static void decodeToFile(String dataToDecode, String filename) throws java.io.IOException {
		...
		UtilBase64.OutputStream bos = null;
		try {
			bos = new UtilBase64.OutputStream(new java.io.FileOutputStream(filename), UtilBase64.DECODE);
	...
	public static byte[] decodeFromFile(String filename) throws java.io.IOException {
		...
		UtilBase64.InputStream bis = null;
		try {
			...
			// Open a stream
			bis = new UtilBase64.InputStream(new java.io.BufferedInputStream(new java.io.FileInputStream(file)), UtilBase64.DECODE);
	...
	public static String encodeFromFile(String filename) throws java.io.IOException {
		...
		try {
			...
			// Open a stream
			bis = new UtilBase64.InputStream(new java.io.BufferedInputStream(new java.io.FileInputStream(file)), UtilBase64.ENCODE);
			...
			// Save in a variable to return
			encodedData = new String(buffer, 0, length, UtilBase64.PREFERRED_ENCODING);
	...
	public static void encodeFileToFile(String infile, String outfile) throws java.io.IOException {
		...
		String encoded = UtilBase64.encodeFromFile(infile);
	...
	public static void decodeFileToFile(String infile, String outfile) throws java.io.IOException {
		...
		byte[] decoded = UtilBase64.decodeFromFile(infile);
		java.io.OutputStream out = null;
		try {
			out = new java.io.BufferedOutputStream(new java.io.FileOutputStream(outfile));
			out.write(decoded);
	...
	/**
	 * A {@link UtilBase64.InputStream} will read data from another <tt>java.io.InputStream</tt>, given in the constructor, and encode/decode to/from Base64 notation on the fly.
	 *
	 * @see UtilBase64
	 * @since 1.3
	 */
	public static class InputStream extends java.io.FilterInputStream {
		...
		/**
		 * Constructs a {@link UtilBase64.InputStream} in DECODE mode.
		 *
		 * @param in
		...
		/**
		 * Constructs a {@link UtilBase64.InputStream} in either ENCODE or DECODE mode.
		 * <p>
		 * Valid options:
		 * 
		 * <pre>
		 ...
		 * @param options
		 *            Specified options
		 * @see UtilBase64#ENCODE
		 * @see UtilBase64#DECODE
		 * @see UtilBase64#DO_BREAK_LINES
		 * @since 2.0
		 */
	...
	/**
	 * A {@link UtilBase64.OutputStream} will write data to another <tt>java.io.OutputStream</tt>, given in the constructor, and encode/decode to/from Base64 notation on the fly.
	 *
	 * @see UtilBase64
	 * @since 1.3
	 */
	public static class OutputStream extends java.io.FilterOutputStream {
		...
		/**
		 * Constructs a {@link UtilBase64.OutputStream} in ENCODE mode.
		 *
		 * @param out
		...
		/**
		 * Constructs a {@link UtilBase64.OutputStream} in either ENCODE or DECODE mode.
		 * <p>
		 * Valid options:
		 * 
		 * <pre>
			...
			// Else, Decoding
				...
				if (decodabet[theByte & 0x7f] > WHITE_SPACE_ENC) {
					...
					if (position >= bufferLength) { // Enough to output.
						...
						int len = UtilBase64.decode4to3(buffer, 0, b4, 0, options);
						out.write(b4, 0, len);
						position = 0;
					} // end if: enough to output
